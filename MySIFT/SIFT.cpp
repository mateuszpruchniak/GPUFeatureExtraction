...
// and intervals
SIFT::SIFT(char* filename, int octaves, int intervals)
{
	// Load the image
		...
		for(j=1;j<m_numIntervals+1;j++)
		{
			...
			for(xi=1;xi<m_gList[i][j]->width-1;xi++)
			{
				for(yi=1;yi<m_gList[i][j]->height-1;yi++)
				{
					...
					cvSetReal2D(orientation[i][j-1], yi, xi, ori);
				}
			}
...
void SIFT::ExtractKeypointDescriptors()
{
	...
	for(i=0;i<m_numOctaves;i++)
	{
		for(j=1;j<m_numIntervals+1;j++)
		{
			...
			// Do the calculations
			for(float ii=0.5;ii<width-0.5;ii++)
			{
				for(float jj=0.5;jj<height-0.5;jj++)
				{
					IplImage* tmp = cvCreateImage(cvSize(1, 1), 32, 1);

					cvGetRectSubPix( m_gList[i][j], tmp, cvPoint2D32f(ii+1.0,jj)); 
					double dy1 = cvGetReal2D( tmp , 0, 0);
					cvGetRectSubPix( m_gList[i][j], tmp, cvPoint2D32f(ii-1.0,jj)); 
					double dy2 = cvGetReal2D( tmp , 0, 0);
					
					double dy =  dy1 - dy2;
					
					cvGetRectSubPix( m_gList[i][j], tmp, cvPoint2D32f(ii,jj+1.0)); 
					double dx1 = cvGetReal2D( tmp , 0, 0);
					cvGetRectSubPix( m_gList[i][j], tmp, cvPoint2D32f(ii,jj+1.0)); 
					double dx2 = cvGetReal2D( tmp , 0, 0);
...
*/
			...
			{
				cvSetReal2D(imgInterpolatedMagnitude[i][j-1], 0, iii, 0.0);
				cvSetReal2D(imgInterpolatedMagnitude[i][j-1], height, iii, 0.0);
				cvSetReal2D(imgInterpolatedOrientation[i][j-1], 0, iii, 0.0);
				cvSetReal2D(imgInterpolatedOrientation[i][j-1], height, iii, 0.0);
			}

			for(unsigned int jjj=0;jjj<height+1;jjj++)
			{
				cvSetReal2D(imgInterpolatedMagnitude[i][j-1], jjj, 0, 0.0);
				cvSetReal2D(imgInterpolatedMagnitude[i][j-1], jjj, width, 0.0);
				cvSetReal2D(imgInterpolatedOrientation[i][j-1], jjj, 0, 0.0);
				cvSetReal2D(imgInterpolatedOrientation[i][j-1], jjj, width, 0.0);
			}
...
void SIFT::ShowKeypoints()
{
	...
	for(int i=50;i<100;i++)
	{
		...
		cvLine(img, cvPoint(kp.xi, kp.yi), cvPoint(kp.xi+kp.mag[0]*100*cos(kp.orien[0]), kp.yi+ kp.mag[0]*100*sin((double)kp.orien[0])), CV_RGB(255,255,255), 1);
	}
	...
	// Save the image
	char* filename = new char[200];
	sprintf(filename, "C:\\Users\\Mati\\Pictures\\extrema_oct_%d_scale.jpg", fileName );
...
void SIFT::FindMatches( vector<Descriptor> keysToFind )
{
	...
	for ( int i = 0; i < keysToFind.size() ; i++) {
		...
		if (match.xi != -1) {
			...
			//cout << "find " << match.xi << " " << match.yi << endl;
			matchCount++;
...
Descriptor SIFT::CheckForMatch( Descriptor k1, vector<Descriptor> keysList )
{
    ...
    */
	...
	if ( distsq2 - distsq1 > 0.2 * distsq1)
      return minkey;
	return res;
}
