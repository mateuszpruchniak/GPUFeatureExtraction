__kernel void ckMax(__global uchar* ucSource,__global uchar* ucDest,
	    ...
	    if( get_global_id(0) > r && get_global_id(0) < ImageWidth-r && get_global_id(1)-1 > r && get_global_id(1)-1 < get_global_size(1)-r)
		{
			...
			for(int j = -r ; j <= r; j++ ) //y
			{
				for(int i = -r ; i <= r; i++ ) //x
				{
					...
					fMaxEstimate[0] = fMaxEstimate[0] >= tmp[0] ? fMaxEstimate[0] : tmp[0];					
					fMaxEstimate[1] = fMaxEstimate[1] >= tmp[1] ? fMaxEstimate[1] : tmp[1];						
					fMaxEstimate[2] = fMaxEstimate[2] >= tmp[2] ? fMaxEstimate[2] : tmp[2];	
				}		
			}
			
			uchar4 pix;
			pix.x = (char)fMaxEstimate[0];
			pix.y = (char)fMaxEstimate[1];
			pix.z = (char)fMaxEstimate[2];
